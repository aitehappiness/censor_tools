...
ap = argparse.ArgumentParser(description='script for bk detection test')
...
# pulp recall
def terror_recall(cnf_matrix):
    return float(cnf_matrix[0][0]) / (np.sum(cnf_matrix[0]) + eps)

# pulp precision
def terror_precision(cnf_matrix):
...
def main():
    ...
    tr=terror_recall(conf_matrix)
    tp=terror_precision(conf_matrix)
    nr=normal_recall(conf_matrix)
    np=normal_precision(conf_matrix)

    print('confusion matrix:\n%s'%conf_matrix)
    print('accuracy:         %s'%acc)
    print('terror_recall:      %s'%tr)
    print('terror_precision:   %s'%tp)
    print('normal_recall:    %s'%nr)
    print('normal_precision: %s'%np)

    labels = ['terror', 'normal']
